...
function addWeightAnnotation() {
  var date = new Date();
  var time = `${date.getFullYear()}-${String(date.getMonth()).padStart(2,'0')}-${String(date.getDay()).padStart(2,'0')}T${String(date.getHours()).padStart(2,'0')}:${String(date.getMinutes()).padStart(2,'0')}:${String(date.getSeconds()).padStart(2,'0')}+${String(date.getTimezoneOffset()/60).padStart(2,'0')}:${String(date.getTimezoneOffset()%60).padStart(2,'0')}`
  var text = document.getElementById("annotation").value
  var annotation = `cphrampus3,${time},${text}`
