...
client.request(`/MedicationRequest?patient=${client.patient.id}`, {
  ...
  function addWeightAnnotation() {
    ...
    client.patient.request("Observation?code=http://loinc.org|29463-7", {flat: true})
    .then(observations => {

      //get latest
      observation = observations[0];
    // add note obj to saved weight obv
    observation.note = [annotation, ...(observation.note||[])];
  
    // post back updated object
    client.update(observation).then(x=>console.log(x));
    });
  }
